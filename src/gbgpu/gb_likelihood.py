...
# ---------------------------------------------------------------------------
# STFT (Fresnel) engine (wraps gbgpu.gbcomps.STFTGBComputations)
# ---------------------------------------------------------------------------


class STFTBandLikelihoodEngine(TwoQuadraturePhaseMaxMixin):
    ...
    call runs under that split's device context. The single-split case
    degenerates to one iteration with an identity remap. Cross-device gathers
    of the per-split outputs into the full-length result arrays are untested
    on multi-GPU.

    The STFT kernels return raw COMPLEX inner products; the engine stashes
    their real parts on :attr:`d_h_out` / :attr:`h_h_out` (the protocol/mixin
    convention shared with the FD and WDM engines) and keeps the raw values
    on :attr:`d_h_out_cmplx` / :attr:`h_h_out_cmplx` for cross-checks.
    ``phase_maximize=True`` routes through the two-quadrature mixin (the
    complex output would allow a single-call analytic rotation, but the
    carrier sign convention has not been validated yet -- documented
    follow-up).
    ...
    there is no runtime override here. ``N_vals`` / ``waveform_kwargs`` are
    FD-specific and ignored.
    """
    ...
    def _central_bin_keep(self, *params_phys_arrays):
        """Central-bin bounds mask (WDM parity): each source's ``f0`` FD bin
        must sit inside the active band ``[ind_min, ind_max]``. The kernels
        clamp every pixel access to the grid, so this is proposal semantics
        (reject band-escapees with ``-1e300``), not crash safety."""
        df = self.basis_settings.df
        ind_min = self.basis_settings.ind_min
        ind_max = self.basis_settings.ind_max
        keep = None
        for p in params_phys_arrays:
            bin_c = (p[:, 1] / df).astype(int)
            k = (bin_c >= ind_min) & (bin_c <= ind_max)
    ...
    def get_ll(
        ...
        *,
        ...
        if phase_maximize:
            # Two-quadrature analytic maximisation (Python-level; works on the
            # real-part stashes). See TwoQuadraturePhaseMaxMixin.
            return self._get_ll_phase_max(
                buffer_aca, params_phys,
                data_index=data_index, noise_index=noise_index, N_vals=N_vals,
                return_inner_products=return_inner_products,
        ...
        keep = self._central_bin_keep(params_phys)
        self.kept_out = keep

        ll = xp.full(num_bin, -1e300, dtype=xp.float64)
        d_h_c = xp.zeros(num_bin, dtype=xp.complex128)
        h_h_c = xp.zeros(num_bin, dtype=xp.complex128)
        self.phase_angle = None
        ...
        self.d_h_out = d_h_c.real.copy()
        self.h_h_out = h_h_c.real.copy()
        if return_inner_products:
            return ll, self.d_h_out, self.h_h_out, self.phase_angle
        return ll
    ...
    def setup_in_model(self, buffer_aca, params_phys, data_index,
                       N_vals=None) -> None:
        """Per-source in-model setup hook (no-op for the STFT comps; see the
        WDM engine's version for the sig-het behavior)."""
        return self.gb_stft_comp.setup_in_model(
            buffer_aca, params_phys, data_index, N_vals=N_vals)

    def clear_in_model(self) -> None:
        return self.gb_stft_comp.clear_in_model()
    ...
    def get_swap_ll(
        ...
        *,
        ...
        keep = self._central_bin_keep(pa, pr)
